...
grep -qF 'GRANT SELECT ON \`Meijendel_bronnen\`.\`v_bron_catalogus\`' "$REMOTE_HELPER" ||
  fail 'viewgrant voor v_bron_catalogus ontbreekt.'
grep -qF 'GRANT SELECT ON \`Meijendel_bronnen\`.\`v_literatuur_overzicht\`' "$REMOTE_HELPER" ||
  fail 'viewgrant voor v_literatuur_overzicht ontbreekt.'
grep -qF 'GRANT SELECT ON \`Meijendel_bronnen\`.\`v_contextdataset_overzicht\`' "$REMOTE_HELPER" ||
  fail 'viewgrant voor v_contextdataset_overzicht ontbreekt.'
grep -qF 'information_schema.SCHEMA_PRIVILEGES' "$REMOTE_HELPER" ||
  fail 'controle op brede schemarechten ontbreekt.'
grep -qF 'information_schema.TABLE_PRIVILEGES' "$REMOTE_HELPER" ||
...
if grep -Eq 'remote ".*docker|docker (exec|ps|run|compose|stats)' "$DEPLOY"; then
