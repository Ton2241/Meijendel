...
grep -Fq 'read -r privilege_count unexpected_count <<< "$privilege_inventory"' "$helper"
grep -Fq 'rechteninventaris heeft een onverwacht formaat' "$helper"
grep -Fq 'docker exec -i "$TARGET_CONTAINER" sh -c' "$helper"
