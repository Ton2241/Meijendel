/* UITLEG
Deze query is bedoeld voor een view/rapport: Tellerkaart.
*/
...
-- Haal het interne ID op van de teller voor efficiënte koppeling
SET @teller_id_intern = (SELECT id FROM tellers WHERE tellercode = @gekozen_code);
-- Stap 3: Uitvoering van een SQL‑statement.


-- 2. DE QUERY
SET SESSION group_concat_max_len = 10000;
-- Stap 4: Leest gegevens uit: `tellers`, `territoria`, `plot_jaar_teller`, `soorten`, `plots`.
...
FROM (
    ...
    SELECT 9, 'Meest geregistreerd', 
        IFNULL((SELECT s.soort_naam 
         ...
         WHERE EXISTS (SELECT 1 FROM plot_jaar_teller pjt WHERE pjt.plot_id = w.plot_id AND pjt.jaar = w.jaar AND pjt.teller_id = @teller_id_intern)
         GROUP BY s.id 
        ...
        IFNULL((SELECT GROUP_CONCAT(soort_naam SEPARATOR ', ')
         ...
         WHERE totaal = (
             ...
             FROM (
                 ...
                 FROM territoria w2
                 WHERE EXISTS (SELECT 1 FROM plot_jaar_teller pjt2 WHERE pjt2.plot_id = w2.plot_id AND pjt2.jaar = w2.jaar AND pjt2.teller_id = @teller_id_intern)
                 GROUP BY w2.soort_id
             ) AS min_sub
         )), 'Geen data'), ''
    UNION ALL
    SELECT 12, 'Totaal records', 
        CAST((SELECT COUNT(*) FROM territoria w 
              WHERE EXISTS (SELECT 1 FROM plot_jaar_teller pjt WHERE pjt.plot_id = w.plot_id AND pjt.jaar = w.jaar AND pjt.teller_id = @teller_id_intern)) AS CHAR), ''
    UNION ALL
    SELECT 13, 'Totaal territoria', 
        IFNULL(CAST((SELECT SUM(w.territoria) FROM territoria w 
              WHERE EXISTS (SELECT 1 FROM plot_jaar_teller pjt WHERE pjt.plot_id = w.plot_id AND pjt.jaar = w.jaar AND pjt.teller_id = @teller_id_intern)) AS CHAR), '0'), ''
    UNION ALL
    SELECT 14, '', '', '' 
    
    -- SECTIE 3: Jaaroverzichten
    UNION ALL
    SELECT 20 + w.jaar, 
           CAST(w.jaar AS CHAR), 
    ...
    WHERE EXISTS (
        ...
        AND pjt.jaar = w.jaar 
        AND pjt.teller_id = @teller_id_intern
    )
    GROUP BY w.jaar
