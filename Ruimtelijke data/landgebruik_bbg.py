...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Bereken landgebruik per plot uit CBS BBG/NBBG GeoPackage."
    )
    ...
    parser.add_argument("--class-field", required=True, help="Veldnaam met landgebruikscode")
    parser.add_argument("--label-field", required=True, help="Veldnaam met landgebruiksnaam")
...
def main():
    ...
    landgebruik = gpd.read_file(
        ...
        layer=args.layer,
        bbox=plot_bounds,
    )[[args.class_field, args.label_field, "geometry"]].copy()

    landgebruik["klasse"] = landgebruik[args.label_field].astype(str).str.strip()
