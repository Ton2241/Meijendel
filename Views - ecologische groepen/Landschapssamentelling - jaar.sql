WITH GeselecteerdeGroepen AS (
    ...
    WHERE evg.groep_code bezetting_percentage 100 != 0 -- Filter op specifieke subgroepen
...
JaarData AS (
    ...
    FROM waarnemingen w
    WHERE w.jaar = 2025
      AND w.plot_id NOT IN (3503, 3514)
),
GroepTotalenPerPlot AS (
    ...
    SELECT 
        ...
        SUM(SUM(jd.territoria)) OVER(PARTITION BY jd.plot_id) as plot_totaal
    FROM JaarData jd
    JOIN GeselecteerdeGroepen gg ON jd.soort_id = gg.soort_id
    GROUP BY jd.plot_id, gg.gekozen_code
)
...
SELECT 
    ...
    ROUND((gtp.totaal_groep_territoria / NULLIF(gtp.plot_totaal, 0)) * 100, 0) AS `bezetting_percentage`
...
ORDER BY gtp.plot_id ASC, `bezetting_percentage` DESC;