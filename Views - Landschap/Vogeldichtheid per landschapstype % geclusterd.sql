/* UITLEG
Deze query is bedoeld voor een view/rapport: Vogeldichtheid per landschapstype % geclusterd.
*/
...
landschap_clustering AS (
  -- STAP 1: Clusters definiëren en filteren (Hoogveen ID 4 uitgesloten)
  SELECT 
...
actieve_oppervlakte AS (
  ...
  FROM (SELECT DISTINCT jaar, plot_id FROM territoria) w
...
dichtheid_per_groep AS (
  ...
  INNER JOIN territoria w ON lc.soort_id = w.soort_id
  INNER JOIN geselecteerde_plots gp ON w.plot_id = gp.plot_id
