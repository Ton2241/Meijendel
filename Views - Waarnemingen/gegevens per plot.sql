/* UITLEG
Deze query is bedoeld voor een view/rapport: gegevens per plot.
*/
...
SELECT 
    ...
    IFNULL(MAX(pjt.teller_id), '---') AS Teller_Code,
    CASE 
        WHEN MAX(pjt.teller_id) IS NOT NULL AND COUNT(w.id) > 0 THEN '✅ Volledig'
        WHEN MAX(pjt.teller_id) IS NOT NULL AND COUNT(w.id) = 0 THEN '❌ GEEN DATA (Wel teller)'
        WHEN MAX(pjt.teller_id) IS NULL AND COUNT(w.id) > 0 THEN '✅ Geen teller (Wel geteld)'
...
-- Koppeling met territoria binnen het bereik
LEFT JOIN territoria w ON p.plot_id = w.plot_id 
