...
const now = new Date();
const appTitle = "Ledenadministratie Vogelwerkgroep Meijendel";
...
function textWidget({ name, id, top, bottom, left, right, text, size = "0.875rem", style = "", color = "#111827", background = "none", align = "LEFT" }) {
  return {
    widgetName: name,
    widgetId: id,
    type: "TEXT_WIDGET",
...
function applyHeader(dsl) {
  ...
  if (stats) {
    ...
    stats.bottomRow = 18;
    stats.leftColumn = 2;
    stats.rightColumn = 62;
    stats.textColor = "#384331";
  }
...
const app = appsmithDb.application.findOne({ _id: ObjectId(appId) });
if (!app) throw new Error(`App ${appId} niet gevonden`);
const page = appsmithDb.newPage.findOne({ _id: ObjectId(pageId) });
if (!page) throw new Error(`Pagina ${pageId} niet gevonden`);

appsmithDb.codexAppBackups.insertOne({
  appId,
  createdAt: now,
  reason: "Voor VWG uitstraling en app titel",
  app,
});
appsmithDb.codexPageBackups.insertOne({
  pageId,
  createdAt: now,
  reason: "Voor VWG header styling",
  page,
});

appsmithDb.application.updateOne(
  { _id: ObjectId(appId) },
  { $set: { name: appTitle, updatedAt: now } },
);
...
const updatedApp = appsmithDb.application.findOne({ _id: ObjectId(appId) });
printjson({
  appName: updatedApp.name,
  pageTitle: findWidget(updatedPage, "txtTitel").text,
  tabsTop: findWidget(updatedPage, "tabsLedenadministratie").topRow,
  headerTitle: findWidget(updatedPage, "txtVwgTitle").text,
});
