...
function applyDetailLayout(dsl) {
  ...
  infoCanvas.children.push(
    textWidget({
      ...
      left: 49,
      right: 60,
      text: "{{(() => { const r = tblTellers.selectedRow || {}; return `${r.tellercode || '-'}\\n${r.soort_lid || '-'}`; })()}}",
      color: "#374151",
    }),
    textWidget({
...
appsmithDb.codexPageBackups.insertOne({
  pageId,
  createdAt: now,
  reason: "Voor compact detailpaneel lid",
  page,
});
