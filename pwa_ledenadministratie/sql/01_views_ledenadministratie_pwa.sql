/* Views voor de zelfstandige PWA ledenadministratie.
   Deze views zijn read-only en hebben geen Appsmith-afhankelijkheid.
*/

CREATE OR REPLACE VIEW pwa_teller_lijst AS
SELECT
  t.id,
  t.tellercode,
...
CREATE OR REPLACE VIEW pwa_teller_datakwaliteit AS
SELECT
  t.id,
  t.tellercode,
  TRIM(CONCAT_WS(' ', NULLIF(t.voornaam, ''), NULLIF(t.tussenvoegsel, ''), NULLIF(t.achternaam, ''))) AS naam,
...
CREATE OR REPLACE VIEW pwa_teller_telhistorie AS
SELECT
  t.id AS teller_id,
  t.tellercode,
  TRIM(CONCAT_WS(' ', NULLIF(t.voornaam, ''), NULLIF(t.tussenvoegsel, ''), NULLIF(t.achternaam, ''))) AS naam,
...
CREATE OR REPLACE VIEW pwa_actieve_tellers_per_jaar AS
SELECT
  pjt.jaar,
